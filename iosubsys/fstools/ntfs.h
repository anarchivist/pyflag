 ...
 */
typedef struct {
	u_int8_t	upd_val[2];	// what they should be 
	u_int8_t	upd_seq;	// array of size 2*(upd_cnt-1) w/orig vals
} ntfs_upd;
...
typedef struct {
	...
	u_int8_t	lsn[8];				// 8  $LogFile Sequence Number
	u_int8_t	seq[2];				// 16
	u_int8_t	link[2];			// 18
	u_int8_t	attr_off[2];		// 20
	u_int8_t	flags[2];			// 22
	u_int8_t	size[4];			// 24
	u_int8_t	alloc_size[4];		//28
	u_int8_t	base_ref[6];		// 32 
	u_int8_t	base_seq[2];		// 38 
	u_int8_t	next_attrid[2];	// 40 The next id to be assigned
	u_int8_t	f1[2];			// XP Only
	u_int8_t	entry[4];		// XP Only - Number of this entry
} ntfs_mft;
...
typedef struct {
	u_int8_t	type[4];
	u_int8_t	len[4];				// 4 - length including header
	u_int8_t	res;				// 8 - resident flag
	u_int8_t	nlen;				// 9 - name length
	u_int8_t	name_off[2];		// 10 - offset to name 
	u_int8_t	flags[2];			// 12  
	u_int8_t	id[2];				// 14 - unique identifier

	union {		
		/* Resident Values */
		struct {
			u_int8_t	ssize[4];		// 16 - size of content
			u_int8_t	soff[2];		// 20 - offset to content (after name)
			u_int8_t	idxflag[2];		// 22 - indexed flag 
		} r;
		/* Non-resident Values */
		struct {
			u_int8_t	start_vcn[8];	// 16 - starting VCN of this attribute
			u_int8_t	last_vcn[8];	// 24
			u_int8_t	run_off[2];		// 32 - offset to the data runs (after name)
			u_int8_t	compusize[2];	// 34 - compression unit size (2^x)
			u_int8_t	f1[4];			// 36
			u_int8_t	alen[8];		// 40	allocated size of stream
			u_int8_t	ssize[8];		// 48	actual size of stream
			u_int8_t	initsize[8];	// 56   initialized steam size
		} nr;
	} c;
} ntfs_attr;
...
typedef struct {
	...
	u_int8_t	ctime[8];	/* mod time for FILE record (MFT Entry) */
 ...
 */
typedef struct {
	u_int8_t	begin_off[4];	/* offset to seq of idx entries */
	u_int8_t	end_off[4];		/* offset to end of seq of idx entries */
	u_int8_t	buf_off[4];		/* offset to end of idx buffer */
	u_int8_t	flags[4];		
} ntfs_idxelist;
...
typedef struct {
	...
	u_int8_t	collation_rule[4];
	u_int8_t	idxalloc_size_b[4];	/* index alloc size in bytes */
	u_int8_t	idx_size_c;		/* index alloc size in clusters */
	u_int8_t	pad[3];
	ntfs_idxelist	list;
} ntfs_idxroot;
...
typedef struct {
	...
	u_int8_t	lsn[8];			/*  $LogFile Sequence Number */
...
typedef struct {
	u_int8_t	file_ref[6]; /* file reference (invalid for last entry) */
	u_int8_t	seq_num[2]; /* file reference (invalid for last entry) */
	u_int8_t	idxlen[2];		/* length of the index entry */
...
typedef struct {
	u_int8_t	objid1[8];		/* object id of file or directory */
	u_int8_t	objid2[8];		
	u_int8_t	orig_volid1[8];	/* id of "birth" volume */
	u_int8_t	orig_volid2[8];	
	u_int8_t	orig_objid1[8];	/* original object id */
	u_int8_t	orig_objid2[8];	
	u_int8_t	orig_domid1[8];	/* id of "birth" domain */
	u_int8_t	orig_domid2[8];	
} ntfs_attr_objid;
...
extern void
ntfs_data_walk(NTFS_INFO *, FS_DATA *, int, FS_FILE_WALK_FN, char *);
