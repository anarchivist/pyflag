/*************************************************
   ...
   cumulative sum of all chunk sizes for each uncompressed block. This
   is used for quickly searching the sgzipped file to locate a single
   block. 

   NOTE: The index does not store the absolute index into the file,
   but the cumulative sum of all chunk sizes, to find the corrent
   offset into the file, we must add the size of the header and the
   total overheads per chunk, e.g.: index[x] yields an offset of
   index[x]+sizeof(header) + x*sizeof(unsigned int)+1, if the chunk
   sizes are specified as unsigned ints.
