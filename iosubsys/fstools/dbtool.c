  ...
  FS_DATA *fs_data)
{
	...
	cleanup(fs_dent->name);
	cleanup(fs_dent->path);
...
/*
 * sanitise data as req'd for mysql
 */
void cleanup(char *name) {
	int i;
...
//					     FS_FLAG_FILE_AONLY | FS_FLAG_DATA_CONT | FS_FLAG_META_ALLOC | FS_FLAG_META_UNALLOC | FS_FLAG_FILE_NOABORT,
	  ...
	  if(fs_inode->size > 0) {
	    ...
	    fs->file_walk(fs, fs_inode, 0, 0, 
			  (FS_FLAG_FILE_AONLY | FS_FLAG_FILE_RECOVER | FS_FLAG_FILE_NOSPARSE | FS_FLAG_FILE_NOABORT),
//FS_FLAG_FILE_AONLY | FS_FLAG_DATA_CONT | FS_FLAG_FILE_META |
//			   FS_FLAG_META_ALLOC | FS_FLAG_META_UNALLOC| FS_FLAG_FILE_NOABORT ),
			  print_addr, (char *)&ptr);
	    ...
	    int size, int flags, char *ptr)
{
  ...
  // skip if no data
  if(size > 0) {
    /* indirect block pointer, dont add to list */
    //if (flags & FS_FLAG_FILE_META) {
    //  return WALK_CONT;
    //}
    
    //	if( ((fs->ftype & FSMASK) == NTFS_TYPE) && ((flags & FS_FLAG_DATA_CONT) == 0)) {
    //if(((fs->ftype & FSMASK) == NTFS_TYPE) && !(flags & FS_FLAG_DATA_ALLOC) && !(flags & FS_FLAG_DATA_UNALLOC)) {
...
//	printf("INSERT INTO block_%s VALUES\n", tbl_name);
...
main(int argc, char **argv) 
{
	...
	if(!io) {
	  RAISE(E_IOERROR,NULL,"Could not set io subsystem %s",io_subsys);
	};
	 ...
	 */
	//printf("INSERT INTO file_%s VALUES\n", tbl_name);
	fprintf(stderr, "Loading Directory Entries\n");
	fs->dent_walk(fs, fs->root_inum, f_flags, print_dent, (char *)0); 
	//printf(";\n");
...
void create_tables(char *name) {
	...
	// these fields are all joined in 'inode' now...
	//"	`inode` INT NOT NULL,\n"
	//"	`type` INT NOT NULL,\n"
	//"	`id` INT NOT NULL,\n"
