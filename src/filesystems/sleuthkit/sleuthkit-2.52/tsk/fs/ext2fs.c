        ...
        tsk_fprintf(stderr,
            ...
            tsk_getu16(fs->endian, dino->i_uid) + (tsk_getu16(fs->endian,
                ...
                dino->i_gid) + (tsk_getu16(fs->endian,
                    dino->i_gid_high) << 16),
            tsk_getu32(fs->endian, dino->i_mtime),
            tsk_getu32(fs->endian, dino->i_atime),
            tsk_getu32(fs->endian, dino->i_ctime),
            tsk_getu32(fs->endian, dino->i_dtime));
...
ext2fs_dinode_copy(EXT2FS_INFO * ext2fs, TSK_FS_INODE * fs_inode)
{
    ...
    fs_inode->time2.ext2.dtime = tsk_getu32(fs->endian, in->i_dtime);
...
static TSK_FS_INODE *
ext2fs_inode_lookup(TSK_FS_INFO * fs, TSK_INUM_T inum)
{
...
uint8_t
ext2fs_inode_walk(TSK_FS_INFO * fs, TSK_INUM_T start_inum, TSK_INUM_T end_inum,
...
uint8_t
ext2fs_block_walk(TSK_FS_INFO * fs, TSK_DADDR_T start_blk, TSK_DADDR_T end_blk,
    ...
    TSK_FS_FILE_WALK_CB action, void *ptr)
{
    ...
    if (addr == 0) {
        ...
        int retval;
        size_t rounded_size = roundup(read_count, EXT2FS_DEV_BSIZE);
        if ((flags & TSK_FS_FILE_FLAG_AONLY) == 0) {
            ssize_t cnt;
...
ext2fs_file_walk_indir(TSK_FS_INFO * fs, TSK_DATA_BUF * buf[],
    TSK_OFF_T length, TSK_DADDR_T addr, int level, int flags,
    TSK_FS_FILE_WALK_CB action, void *ptr)
{
 ...
 * @param type Attribute type to read and analyze (does not apply to Ext2)
 * @param id Attribute id to read and analyze (does not apply to Ext2)
...
ext2fs_fsstat(TSK_FS_INFO * fs, FILE * hFile)
{
    ...
    if (tsk_getu32(fs->endian, sb->s_last_orphan)) {
        ...
        while (or_in) {
            ...
            or_in = (uint32_t) fsi->time2.ext2.dtime;
...
typedef struct {
    FILE *hFile;
    int idx;
    TSK_DADDR_T indirl[EXT2FS_INDIR_SIZ];
 ...
 */
static uint8_t
ext2fs_istat(TSK_FS_INFO * fs, FILE * hFile, TSK_INUM_T inum, TSK_DADDR_T numblock,
    int32_t sec_skew)
{
    EXT2FS_INFO *ext2fs = (EXT2FS_INFO *) fs;
    TSK_FS_INODE *fs_inode;
    ...
    if (sec_skew != 0) {
        ...
        if (fs_inode->time2.ext2.dtime) {
            fs_inode->time2.ext2.dtime -= sec_skew;
            tsk_fprintf(hFile, "Deleted:\t%s", ctime(&fs_inode->time2.ext2.dtime));
            fs_inode->time2.ext2.dtime += sec_skew;
        }
