 ...
 */
static uint8_t
getFAT(FATFS_INFO * fatfs, TSK_DADDR_T clust, TSK_DADDR_T * value)
{
    uint8_t *ptr;
    uint16_t tmp16;
    TSK_FS_INFO *fs = (TSK_FS_INFO *) & fatfs->fs_info;
...
int8_t
is_clustalloc(FATFS_INFO * fatfs, TSK_DADDR_T clust)
{
    TSK_DADDR_T content;
 ...
 */
int8_t
is_sectalloc(FATFS_INFO * fatfs, TSK_DADDR_T sect)
{
...
*/
uint8_t
fatfs_block_walk(TSK_FS_INFO * fs, TSK_DADDR_T start_blk,
    TSK_DADDR_T end_blk, TSK_FS_BLOCK_FLAG_ENUM flags,
    ...
    for (; addr <= end_blk; addr += fatfs->csize) {
        ...
        if ((flags & myflags) == myflags) {
            ...
            if (end_blk - addr + 1 < fatfs->csize)
                read_size = (size_t) (end_blk - addr + 1);
            else
    ...
    fatfs_dentry * in, TSK_DADDR_T sect, TSK_INUM_T inum)
{
    ...
    if ((in->attrib & FATFS_ATTR_LFN) == FATFS_ATTR_LFN) {
        ...
        fs_inode->size = (TSK_OFF_T) tsk_getu32(fs->endian, in->size);
    ...
    if ((in->attrib & FATFS_ATTR_DIRECTORY) &&
        ...
        if (fs_inode->flags & TSK_FS_INODE_FLAG_ALLOC) {
         ...
         * directory */
           ...
           if (is_clustalloc(fatfs, FATFS_DENTRY_CLUST(fs, in)) == 1) 
               fs_inode->size = 0;
           else
               fs_inode->size = fatfs->csize << fatfs->ssize_sh;
        }
    }
...
inode_walk_file_act(TSK_FS_INFO * fs, TSK_DADDR_T addr, char *buf,
...
fatfs_inode_walk(TSK_FS_INFO * fs, TSK_INUM_T start_inum,
    TSK_INUM_T end_inum, TSK_FS_INODE_FLAG_ENUM flags,
    TSK_FS_INODE_WALK_CB action, void *ptr)
{
    char *myname = "fatfs_inode_walk";
    FATFS_INFO *fatfs = (FATFS_INFO *) fs;
    TSK_INUM_T inum;
    TSK_FS_INODE *fs_inode;
    TSK_DADDR_T sect, ssect, lsect, base_read;
    fatfs_dentry *dep;
    ...
    if ((sect < fatfs->firstclustsect) &&
        ...
        for (; sect <= lsect && sect < fatfs->firstclustsect; sect++) {
            ...
            for (didx = 0; didx < fatfs->dentry_cnt_se;
                ...
                if ((retval2 =
                        fatfs_dinode_copy(fatfs, fs_inode, dep, sect,
                            inum)) != TSK_OK) {
 ...
 */
static TSK_FS_INODE *
fatfs_inode_lookup(TSK_FS_INFO * fs, TSK_INUM_T inum)
{
    FATFS_INFO *fatfs = (FATFS_INFO *) fs;
    TSK_FS_INODE *fs_inode;
    ssize_t cnt;
    ...
    if (fatfs_isdentry(fatfs, fatfs->dep)) {
        if ((retval =
                fatfs_dinode_copy(fatfs, fs_inode, fatfs->dep, sect,
                    inum)) != TSK_OK) {
 ...
 * in fs_inode. The run will list the starting CLUSTER and length in CLUSTERS
 *
 * @param fatfs File system to analyze
 * @param fs_inode File to process and structore to store results in
 * @param a_flags Flags used during processing (only RECOVER is used)
 *
 * @returns 1 on error and 0 on success
 */
static uint8_t
fatfs_make_data_run(FATFS_INFO * fatfs, TSK_FS_INODE * fs_inode,
    TSK_FS_FILE_FLAG_ENUM a_flags)
{
    TSK_FS_INFO *fs = (TSK_FS_INFO *) fatfs;
    TSK_DADDR_T clust = fs_inode->direct_addr[0];
    TSK_OFF_T size = fs_inode->size;

    // see if we have already loaded the runs
    if (fs_inode->attr_state == TSK_FS_INODE_ATTR_STUDIED) {
        if ((fs_inode->attr_flags != a_flags) && (fs_inode->attr != NULL)) {
            tsk_fs_data_free(fs_inode->attr);
            fs_inode->attr = NULL;
        }
        else {
    ...
    else if (fs_inode->attr_state == TSK_FS_INODE_ATTR_ERROR) {
        return 1;
    }
    // not sure why this would ever happen, but...
    else if (fs_inode->attr != NULL) {
        tsk_fs_data_free(fs_inode->attr);
        fs_inode->attr = NULL;
    }
    ...
    if ((clust > (fatfs->lastclust)) &&
        ...
        fs_inode->attr_state = TSK_FS_INODE_ATTR_ERROR;
        tsk_error_reset();
    ...
    /* We need to handle the root directory specially because it 
     * is not in the FAT,
     * but we cannot do it here because we can't store clusters 
     */
    if ((fs->ftype != TSK_FS_INFO_TYPE_FAT_32) && (clust == 1)) {
        ...
        tsk_errno = TSK_ERR_FS_INODE_INT;
        snprintf(tsk_errstr, TSK_ERRSTR_L,
            "fatfs_make_data_run: Cannot process root directory in this function");
        return 1;
    }
     ...
     */
        ...
        (a_flags & TSK_FS_FILE_FLAG_RECOVER)) {
        TSK_DADDR_T sbase;
        TSK_DADDR_T startclust = clust;
        TSK_OFF_T recoversize = fs_inode->size;
        int retval;
        TSK_FS_DATA_RUN *data_run = NULL;
        TSK_FS_DATA_RUN *data_run_head = NULL;
    ...
    /* Normal cluster chain walking */
        ...
        while ((clust & fatfs->mask) > 0 && (int64_t) size > 0 &&
            ...
            data_run->len++;
            full_len_s += fatfs->csize;
            size -= (fatfs->csize * fs->block_size);
    ...
    fs_inode->attr_flags = a_flags & TSK_FS_FILE_FLAG_RECOVER;
    return 0;
}
 ...
 * @param type Attribute type to read and analyze (does not apply to FAT)
 * @param id Attribute id to read and analyze (does not apply to FAT)
 * @param a_st_off Byte offset in the file to start calling the callback on (must be multiple of block size)
    ...
    TSK_FS_FILE_FLAG_ENUM flags, TSK_FS_FILE_WALK_CB action, void *ptr)
{
    ...
    unsigned int i;
    TSK_OFF_T size;
    TSK_DADDR_T clust, sbase;
    uint32_t len;
    TSK_DATA_BUF *data_buf;
    ssize_t cnt;
    TSK_FS_DATA_RUN *data_run_cur;
    int64_t rel_st_offset;      // bytes remaining until we should start to process the clusters

    // clean up any error messages that are lying around
    tsk_error_reset();

    // verify offset is a blocksize multiple
    if (a_st_off % fs->block_size) {
        tsk_errno = TSK_ERR_FS_WALK_RNG;
        snprintf(tsk_errstr, TSK_ERRSTR_L,
            "fatfs_file_walk_off: offset not multiple of block size: %"
            PRIuDADDR, a_st_off);
        return 1;
    }

    if (a_st_off > fs_inode->size) {
        tsk_errno = TSK_ERR_FS_WALK_RNG;
        snprintf(tsk_errstr, TSK_ERRSTR_L,
            "fatfs_file_walk_off: offset (%" PRIuDADDR
            ") larger than file size (%" PRIuDADDR ")", a_st_off,
            fs_inode->size);
        return 1;
    }

    rel_st_offset = (int64_t) a_st_off;
    ...
    /* We need to handle the root directory specially because it is not 
     * in the FAT and has no cluster addresses */
    if ((fs->ftype != TSK_FS_INFO_TYPE_FAT_32)
        ...
        for (i = 0; i < snum; i++) {
            ...
            // make sure we are past the offset that they supplied
            if (rel_st_offset > 0) {
                rel_st_offset -= fatfs->ssize;
                continue;
            }
            ...
            if ((flags & TSK_FS_FILE_FLAG_AONLY) == 0) {
                ...
                retval =
                    action(fs, fatfs->rootsect + i, NULL,
                    fatfs->ssize, myflags, ptr);
            }
    ...
    /* Check if we already tried to or did load this file */
    if (fs_inode->attr_state == TSK_FS_INODE_ATTR_ERROR) {
        tsk_error_reset();
        tsk_errno = TSK_ERR_FS_INODE_INT;
        snprintf(tsk_errstr, TSK_ERRSTR_L,
            "fatfs_file_walk: previous error processing file");
        if (data_buf)
            tsk_data_buf_free(data_buf);
        return 1;
    }
    else if (fs_inode->attr_state == TSK_FS_INODE_ATTR_EMPTY) {
        if (fatfs_make_data_run(fatfs, fs_inode, flags)) {
            if (data_buf)
                tsk_data_buf_free(data_buf);
            return 1;
        }
    }
    // make sure the cached runs were created with the same flags
    // as we currently want
    else if ((fs_inode->attr_flags & TSK_FS_FILE_FLAG_RECOVER) !=
    ...
    if (fs_inode->attr == NULL) {
        ...
        tsk_errno = TSK_ERR_FS_INODE_INT;
        snprintf(tsk_errstr, TSK_ERRSTR_L,
            "fatfs_file_walk: Data run structure is NULL");
        if (data_buf)
            tsk_data_buf_free(data_buf);
        return 1;
    }

    data_run_cur = fs_inode->attr->run;
    if (data_run_cur == NULL) {
        if (tsk_verbose)
            fprintf(stderr,
                "run list is empty in the data run structure -- file size is 0");
        return 0;
    }

    // cycle through the run until we find where we can start to process the clusters
    for (; (data_run_cur) && ((int64_t) size > 0);
        data_run_cur = data_run_cur->next) {
        int myflags = TSK_FS_BLOCK_FLAG_CONT;
        int retval;
        int a;
        ...
        if ((rel_st_offset > 0) &&
            ...
            ((TSK_OFF_T) ((data_run_cur->next->offset *
                        (fatfs->csize << fatfs->ssize_sh))) <= a_st_off)) {
            TSK_OFF_T diff =
                (data_run_cur->next->offset -
                data_run_cur->offset) * (fatfs->csize << fatfs->ssize_sh);
            rel_st_offset -= diff;
            size -= diff;
            continue;
        }

        // calculate the starting cluster in this run 
        if (rel_st_offset > 0) {
            // this calculation prevents unneeded seeking in the next for loop
            a = (int) (rel_st_offset / (fatfs->csize << fatfs->ssize_sh));
            rel_st_offset -= a * (fatfs->csize << fatfs->ssize_sh);
            size -= a * (fatfs->csize << fatfs->ssize_sh);
        }
        else {
            a = 0;
        }
        for (; a < data_run_cur->len; a++) {

            clust = data_run_cur->addr + a;
            sbase = FATFS_CLUST_2_SECT(fatfs, clust);
            ...
            for (i = 0; i < fatfs->csize && (int64_t) size > 0; i++) {
                ...
                // See if our starting point is this sector or past
                if (rel_st_offset > 0) {
                    rel_st_offset -= fatfs->ssize;
                    size -= fatfs->ssize;
                    continue;
                ...
                if ((flags & TSK_FS_FILE_FLAG_AONLY) == 0) {
                    retval =
                        action(fs, sbase + i,
                        &data_buf->data[i << fatfs->ssize_sh], len,
                        myflags, ptr);
                }
                else {
                    retval =
                        action(fs, sbase + i, NULL, len, myflags, ptr);
                }
                if (retval == TSK_WALK_STOP) {
                    if (data_buf)
                        tsk_data_buf_free(data_buf);
...
fatfs_file_walk(TSK_FS_INFO * fs, TSK_FS_INODE * fs_inode, uint32_t type,
    ...
    void *ptr)
{
    return fatfs_file_walk_off(fs, fs_inode, type, id, 0, flags, action,
        ptr);
}
    ...
    if (fatfs->fs_info.ftype != TSK_FS_INFO_TYPE_FAT_32) {
        ...
        if (tsk_getu16(fs->endian, sb->a.f32.fsinfo)) {
            cnt =
                ...
                (TSK_DADDR_T) tsk_getu16(fs->endian, sb->a.f32.fsinfo));
    ...
    TSK_FS_INFO_TYPE_ENUM ftype, uint8_t test)
{
    ...
    cnt = tsk_fs_read_random(fs, (char *) fatsb, len, (TSK_DADDR_T) 0);
    if (cnt != len) {
        if (cnt >= 0) {
            tsk_error_reset();
            tsk_errno = TSK_ERR_FS_READ;
    ...
    fs->inum_count =
        (fs->last_block_act + 1 -
        fatfs->firstdatasect) * fatfs->dentry_cnt_se;
