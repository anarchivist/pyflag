 ...
 */
int
hfs_is_bit_b_alloc(TSK_DADDR_T b, uint8_t * alloc_file)
{
    uint8_t this_byte;
// @@@ BC: THIS IS REALLY BAD: We need to check the size of alloc_file
...
hfs_cat_find_node_offset(HFS_INFO * hfs, int nodenum)
{
    ...
    int i;
    TSK_OFF_T bytes;             /* bytes left this extent */
    ...
    bytes =
        ...
        sb->cat_file.extents[i].blk_cnt) * (TSK_OFF_T) fs->block_size;
    r_offs =
        tsk_getu32(fs->endian,
        sb->cat_file.extents[i].start_blk) * (TSK_OFF_T) fs->block_size;
    f_offs = 0;
    ...
    while (f_offs < n_offs) {
        ...
        if (n_offs <= (f_offs + bytes)) {
            ...
            r_offs =
                tsk_getu32(fs->endian,
                sb->cat_file.extents[i].start_blk) *
                (TSK_OFF_T) fs->block_size;
            f_offs += bytes;
            // @@@ This was getu64, but Rob Joyce reported that it should
            // be getu32.  Verify.
            bytes =
                tsk_getu32(fs->endian,
                sb->cat_file.extents[i].blk_cnt) * (TSK_OFF_T) fs->block_size;
...
{
    TSK_FS_INFO *fs = (TSK_FS_INFO *) & (hfs->fs_info);
    TSK_INUM_T highest = 0;
    int nodes;
    int i, j;
    TSK_OFF_T n_offs;               /* offset of the node */
    TSK_OFF_T addr_offs;            /* offset of next record address */
    hfs_btree_node node;
    int num_rec;                /* number of records this node */
    uint8_t r_offs[2];          /* offset of the record in the node */
    TSK_OFF_T read_offs;            /* read offset into node */
...
hfs_load_inode_list(HFS_INFO * hfs)
{
    TSK_FS_INFO *fs = (TSK_FS_INFO *) & (hfs->fs_info);
    int i, j, nodes;
    TSK_DADDR_T n_offs;             /* offset of the node */
    TSK_DADDR_T addr_offs;          /* offset of next record address */
    hfs_btree_node node;
    int num_rec;                /* number of records this node */
    uint8_t r_offs[2];          /* offset of the record in the node */
    TSK_DADDR_T read_offs;          /* read offset into node */
    uint8_t keylen[2];          /* length of this record's key */
    uint8_t rec_type[2];        /* record type */
    uint16_t filetype;
    uint8_t cnid[4];
...
static void
hfs_catalog_lookup(HFS_INFO * hfs, hfs_file * cat, TSK_INUM_T inum)
{
    TSK_FS_INFO *fs = (TSK_FS_INFO *) & (hfs->fs_info);
    htsk_fs_inode_mode_struct *in;
    uint8_t key_len[2];
    TSK_DADDR_T read_offs;
    uint8_t rec_type[2];        /* record type */
    uint32_t filetype;
 ...
 */
uint8_t
hfs_dinode_lookup(HFS_INFO * hfs, TSK_INUM_T inum)
{
    if (tsk_verbose)
        tsk_fprintf(stderr,
 ...
 */
static TSK_FS_INODE *
hfs_inode_lookup(TSK_FS_INFO * fs, TSK_INUM_T inum)
{
    ...
    TSK_FS_FILE_WALK_CB action, void *ptr)
{
    ...
    TSK_OFF_T offs;
    int i;
    size_t bytes_read;             /* bytes read using tsk_fs_read_random */
    TSK_DADDR_T addr;
...
uint8_t
hfs_block_walk(TSK_FS_INFO * fs, TSK_DADDR_T start_blk, TSK_DADDR_T end_blk,
...
uint8_t
hfs_inode_walk(TSK_FS_INFO * fs, TSK_INUM_T start_inum, TSK_INUM_T end_inum,
...
uint8_t
hfs_istat(TSK_FS_INFO * fs, FILE * hFile, TSK_INUM_T inum, TSK_DADDR_T numblock,
    int32_t sec_skew)
{
    HFS_INFO *hfs = (HFS_INFO *) fs;
    ...
    TSK_FS_INFO_TYPE_ENUM ftype, uint8_t test)
{
    ...
    cnt = tsk_fs_read_random(fs, (char *) hfs->fs, len, (TSK_OFF_T) HFS_SBOFF);
    if (cnt != len) {
        if (cnt >= 0) {
