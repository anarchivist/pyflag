...
/**
 * \file ffind_lib.c
 * Contains the library API functions used by the ffind command
 * line tool.
 */
#include "tsk_fs_i.h"
...
extern uint8_t
ntfs_find_file(TSK_FS_INFO *, TSK_INUM_T, uint32_t,
    uint16_t, int, TSK_FS_DENT_TYPE_WALK_CB, void *ptr);


static TSK_INUM_T inode = 0;
...
uint8_t
tsk_fs_ffind(TSK_FS_INFO * fs, uint8_t lclflags, TSK_INUM_T inode_a,
