...
typedef struct {
    ...
    TSK_INUM_T inum;                /* number of above cached cat */

    hfs_btree_header_record *hdr;       /* stored btree header node */

    TSK_OFF_T key;                  /* offset of key for cached inode */
 ...
 * Prototypes
 */
extern uint8_t hfs_dent_walk(TSK_FS_INFO *, TSK_INUM_T, TSK_FS_DENT_FLAG_ENUM,
