...
TSK_FS_DENT *
tsk_fs_dent_realloc(TSK_FS_DENT * fs_dent, size_t namelen)
{
    ...
    TSK_FS_INFO * fs, TSK_FS_DATA * fs_data)
{
    ...
    if (fs_data)
        tsk_fprintf(hFile, "-%"PRIu32"-%"PRIu16"", fs_data->type,
            fs_data->id);
...
tsk_fs_dent_print_mac(FILE * hFile, TSK_FS_DENT * fs_dent,
    TSK_FS_INFO * fs, TSK_FS_DATA * fs_data, const char *prefix)
{
    ...
    /* mode val */
    tsk_fprintf(hFile, "|%u|", (unsigned int) ((fs_inode) ? fs_inode->mode : 0));
    ...
    if (!fs_inode) {
        ...
        tsk_fprintf(hFile, "%d|%"PRIuUID"|%"PRIuGID"|0|", (int) fs_inode->nlink,
