 ...
 */
uint8_t
tsk_hdb_idxaddentry(TSK_HDB_INFO * hdb_info, char *hvalue, TSK_OFF_T offset)
{
    int i;
...
hdb_setupindex(TSK_HDB_INFO * hdb_info, uint8_t htype)
{
...
#ifdef TSK_WIN32
    {
        ...
        if (szLow == 0xffffffff) {
            ...
            tsk_errno = TSK_ERR_HDB_OPEN;
            snprintf(tsk_errstr, TSK_ERRSTR_L,
                     "hdb_setupindex: Error getting size of index file: %s - %d",
                     hdb_info->idx_fname, GetLastError());
            return 1;
        }
 ...
 *
 * @param hdb_info Hash database state information
 ...
 */
int8_t
tsk_hdb_lookup(TSK_HDB_INFO * hdb_info, const char *hash,
                   ...
                   TSK_HDB_LOOKUP_FN action, void *ptr)
{
    ...
    return tsk_hdb_lookup(hdb_info, hashbuf, flags, action, ptr);
}

/**
 * Determine if the hash database that is open has an
 * index that has been created.
 *
 ...
 *
 * @param hdb_info Structure to free.
 */
void
