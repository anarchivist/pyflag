...
sun_load_table_i386(TSK_MM_INFO * mm, sun_dlabel_i386 * dlabel_x86)
{
    ...
    for (idx = 0; idx < tsk_getu16(mm->endian, dlabel_x86->num_parts);
        ...
        TSK_MM_PART_TYPE_ENUM ptype = TSK_MM_PART_TYPE_VOL;

        if (tsk_verbose)
            tsk_fprintf(stderr,
                "load_table_i386: %" PRIu32
                "  Starting Sector: %"PRIu32"  Size: %"PRIu32"  Type: %"PRIu16"\n", idx,
                 tsk_getu32(mm->endian,
        ...
        if ((tsk_getu16(mm->endian, dlabel_x86->part[idx].type) == 5)
            && (tsk_getu32(mm->endian,
...
sun_load_table_sparc(TSK_MM_INFO * mm, sun_dlabel_sparc * dlabel_sp)
{
    ...
    for (idx = 0; idx < tsk_getu16(mm->endian, dlabel_sp->num_parts);
        ...
        // set the entry that covers the entire disk image as DESC
        if ((tsk_getu16(mm->endian, dlabel_sp->part_meta[idx].type) == 5)
            && (part_start == 0))
            ptype = TSK_MM_PART_TYPE_DESC;
...
#define LABEL_BUF_SIZE	512
    ...
    ssize_t cnt;
    TSK_DADDR_T taddr = mm->offset / mm->block_size + SUN_SPARC_PART_SOFFSET;
 ...
 */
uint8_t
sun_part_walk(TSK_MM_INFO * mm, TSK_PNUM_T start, TSK_PNUM_T last, int flags,
...
TSK_MM_INFO *
tsk_mm_sun_open(TSK_IMG_INFO * img_info, TSK_DADDR_T offset)
{
    TSK_MM_INFO *mm;
