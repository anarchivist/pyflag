 ...
 */
static uint8_t
dos_load_ext_table(TSK_MM_INFO * mm, TSK_DADDR_T sect_cur,
    TSK_DADDR_T sect_ext_base, int table)
{
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (dos_is_ext(part->ptype)) {
            ...
            if (NULL == tsk_mm_part_add(mm,
                    (TSK_DADDR_T) (sect_cur + part_start), (TSK_DADDR_T) part_size,
...
dos_load_prim_table(TSK_MM_INFO * mm, uint8_t test)
{
    ...
    char *table_str;
    ssize_t cnt;
    TSK_DADDR_T taddr = mm->offset / mm->block_size + DOS_PART_SOFFSET;
    TSK_DADDR_T max_addr = (mm->img_info->size - mm->offset) / mm->block_size;      // max sector
 ...
 */
uint8_t
dos_part_walk(TSK_MM_INFO * mm, TSK_PNUM_T start, TSK_PNUM_T last, int flags,
 ...
 */
TSK_MM_INFO *
tsk_mm_dos_open(TSK_IMG_INFO * img_info, TSK_DADDR_T offset, uint8_t test)
{
    TSK_MM_INFO *mm;
