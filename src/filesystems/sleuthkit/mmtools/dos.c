...
/* 
 * Load an extended partition table into the structure in TSK_MM_INFO.
 *
 ...
 */
static uint8_t
dos_load_ext_table(TSK_MM_INFO * mm, DADDR_T sect_cur,
    DADDR_T sect_ext_base, int table)
{
    dos_sect sect;
    int i;
    char *table_str;
    ...
    cnt = tsk_mm_read_block_nobuf
        (mm, (char *) &sect, sizeof(sect), sect_cur);
    if (cnt != sizeof(sect)) {
        if (cnt != -1) {
            tsk_error_reset();
    ...
    /* Sanity Check */
    if (tsk_getu16(mm->endian, sect.magic) != DOS_MAGIC) {
        tsk_error_reset();
    ...
    if (NULL == tsk_mm_part_add(mm, (DADDR_T) sect_cur, (DADDR_T) 1,
    ...
    for (i = 0; i < 4; i++) {
         ...
         * ignore CHS */
        uint32_t part_start = tsk_getu32(mm->endian, part->start_sec);
        uint32_t part_size = tsk_getu32(mm->endian, part->size_sec);
        ...
        if (dos_is_ext(part->ptype)) {
            ...
            if (NULL == tsk_mm_part_add(mm,
                    ...
                    (DADDR_T) part_size, TSK_MM_PART_TYPE_DESC,
            ...
            if (NULL == tsk_mm_part_add(mm,
                    (DADDR_T) (sect_cur + part_start), (DADDR_T) part_size,
                    TSK_MM_PART_TYPE_VOL, dos_get_desc(part->ptype), table,
                    i))
                return 1;
        }
    }
 ...
 * data structures in TSK_MM_INFO
 *
 ...
 */
static uint8_t
dos_load_prim_table(TSK_MM_INFO * mm, uint8_t test)
{
    dos_sect sect;
    int i, added = 0;
    char *table_str;
    ...
    /* Sanity Check */
    if (tsk_mm_guessu16(mm, sect.magic, DOS_MAGIC)) {
        tsk_error_reset();
        tsk_errno = TSK_ERR_MM_MAGIC;
        snprintf(tsk_errstr, TSK_ERRSTR_L,
    ...
    if (NULL == tsk_mm_part_add(mm, DOS_PART_SOFFSET, (DADDR_T) 1,
 ...
 */
uint8_t
dos_part_walk(TSK_MM_INFO * mm, PNUM_T start, PNUM_T last, int flags,
 ...
 */
TSK_MM_INFO *
tsk_mm_dos_open(TSK_IMG_INFO * img_info, DADDR_T offset, uint8_t test)
{
    TSK_MM_INFO *mm;
    ...
    mm->mmtype = TSK_MM_INFO_TYPE_DOS;
