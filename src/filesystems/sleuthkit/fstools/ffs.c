 ...
 * */
static TSK_FS_INODE *
ffs_inode_lookup(TSK_FS_INFO * fs, INUM_T inum)
{
 ...
 */
uint8_t
ffs_inode_walk(TSK_FS_INFO * fs, INUM_T start_inum, INUM_T end_inum,
...
uint8_t
ffs_block_walk(TSK_FS_INFO * fs, DADDR_T start_blk, DADDR_T end_blk,
 ...
 */
static SSIZE_T
ffs_file_walk_indir(TSK_FS_INFO * fs, TSK_DATA_BUF * buf[], OFF_T length,
    DADDR_T addr, int level, int flags, TSK_FS_FILE_WALK_CB action,
    void *ptr)
{
...
/*  
 *  
 * flag values: FS_FLAG_FILE_SPARSE, TSK_FS_FILE_FLAG_AONLY, TSK_FS_FILE_FLAG_SLACK
 *  TSK_FS_FILE_FLAG_META
 *
 * If TSK_FS_FILE_FLAG_RECOVER is set, then many error codes are set to _RECOVER
 * so that errors can be suppressed.  NO special recovery logic is used. 
 *
 * Action uses: TSK_FS_BLOCK_FLAG_CONT, TSK_FS_BLOCK_FLAG_META
    ...
    if (tsk_getu32(fs->endian, sb1->cg_ssize_b)) {
        ...
        if ((fs->ftype == TSK_FS_INFO_TYPE_FFS_1)
            ...
            cnt =
                ...
                (DADDR_T) tsk_getu32(fs->endian, sb1->cg_saddr));
            ...
            cnt = tsk_fs_read_block_nobuf
                (fs, (char *) csum1, tsk_getu32(fs->endian,
                    sb2->cg_ssize_b), (DADDR_T) tsk_getu64(fs->endian,
                    sb2->cg_saddr));
 ...
 */
static uint8_t
ffs_istat(TSK_FS_INFO * fs, FILE * hFile, INUM_T inum, DADDR_T numblock,
    int32_t sec_skew)
{
    FFS_INFO *ffs = (FFS_INFO *) fs;
    TSK_FS_INODE *fs_inode;
...
/* Return 1 on error and 0 on success */
uint8_t
ffs_jopen(TSK_FS_INFO * fs, INUM_T inum)
{
    tsk_error_reset();
    tsk_errno = TSK_ERR_FS_FUNC;
...
uint8_t
ffs_jblk_walk(TSK_FS_INFO * fs, DADDR_T start, DADDR_T end, int flags,
...
TSK_FS_INFO *
ffs_open(TSK_IMG_INFO * img_info, SSIZE_T offset,
