...
ext2fs_fsstat(FS_INFO * fs, FILE * hFile)
{
    ...
    fprintf(hFile, "Number of Block Groups: %" PRI_EXT2GRP "\n",
	ext2fs->groups_count);
	...
	if (i != (ext2fs->groups_count - 1)) {
	    fprintf(hFile, "  Free Inodes: %" PRIu16 " (%" PRIu32 "%%)\n",
	    ...
	    fprintf(hFile, "  Free Blocks: %" PRIu16 " (%" PRIu32 "%%)\n",
