...
ntfs_dinode_lookup(NTFS_INFO * ntfs, ntfs_mft * mft, INUM_T mftnum)
{
    ...
    if (mftaddr2_b) {
        ...
        cnt =
            tsk_fs_read_random(&ntfs->fs_info, (char *) mft,
        ...
        cnt = tsk_fs_read_random
            (&ntfs->fs_info,
            (char *) ((uintptr_t) mft + (uintptr_t) mftaddr_len),
    ...
    upd =
        (ntfs_upd *) ((uintptr_t) mft + tsk_getu16(fs->endian,
            mft->upd_off));
    ...
    for (i = 1; i < tsk_getu16(fs->endian, mft->upd_cnt); i++) {
        ...
        uint16_t cur_seq =
            tsk_getu16(fs->endian, (uintptr_t) mft + offset);
        if (cur_seq != sig_seq) {
 ...
 * structure
 *
 * The return value is a list of TSK_FS_DATA_RUN entries of len
 * totlen bytes (only set if non-NULL)
 *
 * Returns NULL on error and for Bad clust: Check tsk_errno to test
 */
static TSK_FS_DATA_RUN *
ntfs_make_data_run(NTFS_INFO * ntfs,
    ntfs_runlist * runlist, OFF_T * totlen)
{
...
ntfs_proc_compunit(NTFS_INFO * ntfs, NTFS_COMP_INFO * comp, int flags,
    TSK_FS_FILE_WALK_CB action, void *ptr, DADDR_T * comp_unit,
    uint32_t comp_unit_size, SSIZE_T * fsize, TSK_DATA_BUF * data_buf)
{
    TSK_FS_INFO *fs = (TSK_FS_INFO *) ntfs;
    ...
    void *ptr)
{
     ...
     */
        ...
        comp_unit =
            (DADDR_T *) tsk_malloc(fs_data->compsize * sizeof(DADDR_T));
        if (comp_unit == NULL) {
...
ntfs_dinode_copy(NTFS_INFO * ntfs, TSK_FS_INODE * fs_inode)
{
    ...
    if (ntfs_proc_attrseq(ntfs, fs_inode, attr,
            ntfs->mft_rsize_b - tsk_getu16(fs->endian, mft->attr_off))) {
        return 1;
    }
    return 0;
}
...
/*
 * return the MFT entry in the generic TSK_FS_INODE format
 *
 * Return NULL on error 
 */
static TSK_FS_INODE *
ntfs_inode_lookup(TSK_FS_INFO * fs, INUM_T mftnum)
{
...
ntfs_proc_sds(TSK_FS_INFO * fs, NTFS_SXX_BUFFER * sds_buffer)
{
    ...
    while (total_bytes_processed < sds_buffer->size) {
        ...
        offset = tsk_getu32(fs->endian, sds->ent_size);
        if (offset % 16) {
...
uint8_t
ntfs_load_sxx_buffer(TSK_FS_INFO * fs, DADDR_T addr, char *buf,
...
ntfs_block_walk(TSK_FS_INFO * fs,
    DADDR_T start_blk, DADDR_T end_blk, TSK_FS_BLOCK_FLAG_ENUM flags,
 ...
 * TSK_FS_INODE_FLAG_USED, TSK_FS_INODE_FLAG_ORPHAN
 *
 * Not used: TSK_FS_INODE_FLAG_UNUSED (Only allocated when needed)
 *
 ...
 */
uint8_t
ntfs_inode_walk(TSK_FS_INFO * fs, INUM_T start_inum, INUM_T end_inum,
...
static uint8_t
print_addr_act(TSK_FS_INFO * fs, DADDR_T addr,
...
uint8_t
ntfs_jopen(TSK_FS_INFO * fs, INUM_T inum)
{
    tsk_error_reset();
    tsk_errno = TSK_ERR_FS_FUNC;
    snprintf(tsk_errstr, TSK_ERRSTR_L,
...
uint8_t
ntfs_jblk_walk(TSK_FS_INFO * fs, DADDR_T start,
    DADDR_T end, int flags, TSK_FS_JBLK_WALK_CB action, void *ptr)
{
    tsk_error_reset();
    tsk_errno = TSK_ERR_FS_FUNC;
    snprintf(tsk_errstr, TSK_ERRSTR_L,
...
/* Return NULL on error */
TSK_FS_INFO *
ntfs_open(TSK_IMG_INFO * img_info, SSIZE_T offset,
    TSK_FS_INFO_TYPE_ENUM ftype, uint8_t test)
{
    ...
    cnt = tsk_fs_read_random(fs, (char *) ntfs->fs, len, (OFF_T) 0);
    if (cnt != len) {
        if (cnt != -1) {
            tsk_error_reset();
            tsk_errno = TSK_ERR_FS_READ;
