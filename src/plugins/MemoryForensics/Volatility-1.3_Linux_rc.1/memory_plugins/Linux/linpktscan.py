...
from vutils import *
from forensics.linux.scan import pkt_scan,pktdmp_scan
import os

class linpktscan(forensics.commands.command):
    ...
    def parser(self):
	    ...
	    help='Extract packets to pcap file',
    ...
    def help(self):
        return  "Scan the address space for network packets"
	...
	(profile, addr_space, symtab, types) = linux_load_and_identify_image( \
        ...
        filesize = os.path.getsize(opts.filename)
	...
	if opts.outfile:
	    pktdmp_scan(flat_address_space,theProfile.abstract_types, filename, start, end, True,opts.outfile) 
        else:
	    pkt_scan(flat_address_space,theProfile.abstract_types, filename, start, end, True) 
