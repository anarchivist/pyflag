...
class Object(object):
    ...
    def get_deep_member(self, list):
        current_object = self
        for i in list:            
            if isinstance(current_object, Pointer):
                if current_object.is_null():
                    return None
                current_object = current_object.dereference()
            current_object = current_object.get_member(i)
