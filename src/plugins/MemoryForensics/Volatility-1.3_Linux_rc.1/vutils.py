...
def linux_load_and_identify_image(op, opts, verbose=False):
    """
    Function checks the parsed options for a valid image and DTB and returns
    (address space, symbol table, types) tuple for that image.
    """
    ...
    if opts.profile is None:
        op.error("Profile is required")
    else:
        try:
            profile = load_profile_from_file(opts.profile)
        except:
            op.error("Invalid or corrupt profile")
    
    if opts.systemmap is None:
        op.error("Symbol Table is required")
    else:
        try:
            symdict = load_symboltable_from_file(opts.systemmap)
            symtable =  SymbolTable(symdict)
        except:
            op.error("Invalid or corrupt Symbol Table")

    pgd = symtable.lookup('swapper_pg_dir')

    try:
        addr_space = linux_nopae_address_space(filename, pgd - 0xc0000000)
    except:
        op.error("Unable to load address space");
    return (profile, addr_space, symtable, types)

def load_profile_from_file(profile_file_name):
    globs = {}
    locs = {}
    execfile(profile_file_name, globs, locs)
    return locs['profile_members']

def load_symboltable_from_file(symbol_table_file):
    dict = {}
    handle = open(symbol_table_file)
    current = handle.readline()
    while (current != ""):
        (addr, t, symbol) = split(current)
        dict[symbol] = int(addr, 16)
        current = handle.readline()
    handle.close()
    return dict

def linux_nopae_address_space(filename, pgd):
    try:
        phyAS = FileAddressSpace(filename)
	addr_space = IA32PagedMemory(phyAS, pgd)
    except:
        addr_space = None
    return addr_space